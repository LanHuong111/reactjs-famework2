...
const Budget = () => {
  ...
  const getDetailSubCategories = async (id) => {
    ...
    const dataSubDetailCategories =
      await subCategoriesService.getDetailSubCategoriesById(id);
      ...
      <div className="relative ">
        {showFormAddBudget && (
          <div className="absolute left-[20%] top-[0%] z-20">
            <CreateBudget
              handleHideForm={setShowFormBudget}
              detailSubcategories={detailSubcategories}
            />
          </div>
        )}
        {showFormAddCategory && (
          <div className="absolute left-[20%] top-[5%] z-10">
            <CreateCategories
              ...
              handleHideForm={setShowFormAddCategory}
            />
          </div>
        )}
        {showFormEdit && (
          <div className="absolute left-[20%] top-[5%] z-10">
            <EditCategories
              getCategories={getCategories}
              categories={categories}
              handleHideForm={setShowFormEdit}
              IdSubCategories={IdSubCategories}
            />
          </div>
        )}

        <div className="flex justify-between gap-5 bg-blue-500 w-full rounded-[5px] p-5">
          <div className="flex gap-2">
            <div>
              <span className="text-[20px] block font-bold text-[#fff]">
                Chọn 1 danh mục chi tiêu
              </span>
              <span className="text-[15px] text-[#fff]">
                Bạn sẽ được thông báo khi chi tiêu <br /> danh mục đạt giới hạn
                tối đa
              </span>
            </div>
          </div>
          <div>
            <button
              ...
              className="bg-[#fff] text-blue-500 rounded-[5px] p-2 w-[150px] font-bold"
            >
              Quay lại
            </button>
          </div>
        ...
        <div className="overflow-auto h-[400px]">
          {categories &&
            categories.map((category) => (
              <div
                key={category._id}
                className="bg-slate-200 my-2 p-3 rounded-[5px] items-center"
              >
                ...
                {category?.subcategories?.map((ChildCategory) => (
                  ...
                  >
                    <div
                      className="bg-[#fff] p-2 rounded-[5px] my-2 cursor-pointer flex justify-between items-center w-[100%]"
                      onClick={() => getDetailSubCategories(ChildCategory._id)}
                    >
                    ...
                    <div className=" flex justify-center gap-3">
                      <IoTrash
                        onClick={() =>
                          handleDeleteSubCategories(ChildCategory._id)
                        }
                      ...
                      <AiFillEdit
                        onClick={() =>
                          handleEditSubCategories(ChildCategory._id)
                        }
                        className="text-[20px] text-blue-500 cursor-pointer"
                      />
                    </div>
          ...
          <div
            className="flex justify-center mt-4 items-center gap-2 cursor-pointer border-2  p-2 rounded-[5px]"
            onClick={() => setShowFormAddCategory(true)}
          >
            <span className="text-[#333] font-semibold">Tạo danh mục</span>
          </div>
        </div>
        <div className="absolute  bottom-[-70px] left-[0] w-full flex justify-center">
          <button
            ...
            className={` w-full text-center  p-2 rounded-[5px] cursor-pointer duration-300 ${
              selectedSubCategory
                ? "bg-[#3B82F6] text-[#fff]"
