...
const ChartHome = () => {
    ...
    const [series, setSeries] = useState([]); // giá tiền của từng danh mục(ngân sách)
    const [percentage, setPercentage] = useState([]);
    const [currentChart, setCurrentChart] = useState('bar'); // State để lưu trữ trạng thái biểu đồ hiện tại
    ...
    useMemo(() => {
        const total = series.reduce((acc, num) => acc + num, 0); // tổng các ngân sách
        const caculatorPercentage = series.map(num => Math.floor((num / total) * 100)); // num là từng ngân sách
            ...
            {data.length === 0 ? (
                ...
                <>
                    <div className="flex justify-center mb-4">
                        <button
                            className={`px-4 py-2 mr-2 ${currentChart === 'bar' ? 'bg-blue-500 text-white' : 'bg-gray-300 text-black'}`}
                            onClick={() => handleChartChange('bar')}
                        >
                            Biểu đồ cột
                        </button>
                        <button
                            className={`px-4 py-2 ${currentChart === 'pie' ? 'bg-blue-500 text-white' : 'bg-gray-300 text-black'}`}
